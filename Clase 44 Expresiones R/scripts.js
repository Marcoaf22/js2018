...
*sintaxis:
    ~ /patron/
    ~ patron = string a buscar sin comillas
*Banderas:
...
*Comodines:
    ~ "." : acepta cualquier caracter en vez del "."   Ej: /lo.em/

    ~ [abc] Listado de caracteres válidos: Entre corchetes se pone una
    ~       lista de los caracteres válidos es individual
            !verlos asi [a,b,c]
        [aeiou] Con esto cogeríamos todas las vocales

    ~ [a-l] : Rangos Entre corchetes si ponemos un guión entre dos caracteres establecemos un rango. [a-z] Todas las letras minúsculas. o numeros
        !Tabla ASCII https://ascii.cl/es/

    ~ Mezcla entre rangos y listas:
        ~Podemos unir los dos anteriores en una sola expresión. [0-5ou] 
        ~Serían números del 0 al 5, la letra "o" y la letra "u"
            !verlo asi [0-5,o,u]
    ~ Cadenas completas:
        ~para establecer una cadena completa debe ir entre paréntesis, si
        ~queremos más palabras irán separadas por un pipe. (lorem|amet) es
        ~válida la palabra "lorem" y la palabra "amet"

*Delimitadores:
    ~ ^ : Antes de este símbolo no puede haber nada, Abosulotamente nada
    ~ $ : Después de este símbolo no puede haber nada, Abosulotamente nada
        ^hola$
*Cantidad:
    ~ {} : llaves: lo que está antes tiene que aparecer la cantidad exacta de veces. Hay tres combinaciones posibles.
        ...
        ^[a-zA-Z]{1,3}@{1}$
    ~ * : Lo que está ANTES del asterisco puede estar, puede no estar
        ~ y "se puede repetir varias veces"  .*@.*\..*
    ~ ?: Lo que está ANTES de la interrogación puede no estar, pero si está solo puede aparecer una vez.
            ^[ae]?$
    ~ + : lo que está ANTES del + tiene que estár "una vez como mínimo"
        A-[0-9]+

*Caracteres:
    ~ \s: Coincide con un carácter de espacio, entre ellos incluidos espacio,
        ~tab, salto de página, salto de linea y retorno de carro.
        ^[a-zA-Z]+\s[a-zA-Z]+$
...
// devuleve boolean si hay coincidencia
console.log(regEx.test(text));

// Otra forma de declarar es la misma
